...
function getFidFromRequest(req: Request): number | null {
  // 1) headerből (MiniAppban így jöhet: x-bbox-fid)
  ...
  // 2) query paramból (böngésző / teszt: /api/me?fid=123)
  try {
...
export async function GET(req: Request) {
  ...
  try {
    // ha még nincs user/user_stats, létrehozzuk
    ...
    return NextResponse.json({
      fid,
      // TS szerint user-ben csak is_og van, ezért cast:
